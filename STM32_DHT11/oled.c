...
void OLED_Refresh() {
    ...
    for (u8 page = 0; page < 8; page++) {
        ...
        for (u8 column = 0; column < 128; column++) {
            OLED_WR_Byte(oled_gram[page * 128 + column], OLED_DATA);
        }
    }
		Delay_50ms(20);
}
...
void OLED_ShowAlert(unsigned char *alert_msg) {
    // ???????????? OLED ?????
    // ???? OLED_ShowString ???????
    // ????:
    OLED_Clear();  // ??
    OLED_ShowString(0, 0, "ALERT!", 16);  // ?? ALERT! ??
    OLED_ShowString(0, 2, alert_msg, 16);  // ??????
    OLED_Refresh_Gram();  // ????
}
//???SSD1306					    
void OLED_Init(void)
